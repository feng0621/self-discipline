...
export function generatePersonalizedPlan(p:TrainingProfile):PersonalizedPlan{
  ...
  const days=names.slice(0,p.daysPerWeek).map((focus,index)=>({day:weekdays[index],focus,duration:p.minutesPerSession,exercises:pool.map((group,i)=>({name:group[(index+i)%group.length],sets:level.sets,reps:i===4?"30–45 秒":level.reps,rest:i===4?"45–60 秒":level.rest}))}));
  ...
  return {title:`${goalName} · 12 周${focus}计划`,summary:`根据 ${p.age} 岁、${p.experience==="beginner"?"入门":p.experience==="intermediate"?"进阶":"高级"}经验、每周 ${p.daysPerWeek} 练和单次 ${p.minutesPerSession} 分钟生成。`,weeklyTarget:`每周 ${p.daysPerWeek} 次力量训练；同一肌群至少间隔 48 小时。`,cardio,progression:"先保证动作标准。当所有工作组都达到次数上限且仍保留 2 次余力，下周增加 1–2 次重复或 2.5%–5% 负重；每第 4 周训练量降低约 25%。",nutrition,days};
}
