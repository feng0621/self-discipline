...
export default function FitnessApp(){
  ...
  return <main className="app">
        ...
        {activeExercises.map((x,i)=>{
          ...
          return <article className={`card ${complete?"complete":""}`} key={x.name}>
            ...
            <div className="cardBody">
              ...
              <div className="name"><small>{x.en}</small><h3>{effectiveSwapped.includes(i)?["墙壁俯卧撑","箱式深蹲","坐姿弹力带划船","蛙式臀桥","高位平板支撑"][i]:x.name}</h3><p>{autoSwapped.includes(i)?"根据不适部位自动替换":x.target}</p><button className="swap" onClick={()=>setSwapped(v=>v.includes(i)?v.filter(n=>n!==i):[...v,i])}>{effectiveSwapped.includes(i)?"恢复原动作":"替换动作"} ↻</button></div>
    ...
    {showSummary&&<div className="decisionOverlay" role="dialog" aria-modal="true"><div className="summaryCard"><p className="kicker">SESSION COMPLETE</p><h2>今天完成得很好。</h2><div className="summaryStats"><span><b>{Math.max(1,Math.round((Date.now()-sessionStartedAt)/60000))}</b><small>分钟</small></span><span><b>{done.length||activeExercises.length}</b><small>动作</small></span><span><b>{activeExercises.reduce((sum,x)=>sum+x.sets,0)}</b><small>组</small></span></div><div className="summaryExercises">{activeExercises.map((x,i)=><span key={x.name}><b>{effectiveSwapped.includes(i)?["墙壁俯卧撑","箱式深蹲","坐姿弹力带划船","蛙式臀桥","高位平板支撑"][i]:x.name}</b><small>{performance[i]?.weight?`${performance[i].weight} ${preferences.unit==="metric"?"kg":"lb"} · `:""}{performance[i]?.reps?`${performance[i].reps} 次`:`${x.sets} 组`}</small></span>)}</div><div className="summaryFeedback">{["太轻松","正合适","太难"].map(x=><button className={feedback===x?"active":""} onClick={()=>void chooseFeedback(x)} key={x}>{x}</button>)}</div><button className="summaryDone" onClick={()=>{setShowSummary(false);setFeedback("")}}>完成并返回</button></div></div>}
