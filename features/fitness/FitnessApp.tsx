...
export default function FitnessApp(){
  ...
  useEffect(()=>{
    if(!user)return;
    const load=async()=>{try{const snapshot=await loadDashboard(user.id);if(snapshot.weight)setWeight(String(snapshot.weight));if(snapshot.waist)setWaist(String(snapshot.waist));if(snapshot.sleep)setSleep(String(snapshot.sleep));if(snapshot.water!==undefined)setWater(snapshot.water);if(snapshot.equipment)setEquipment(snapshot.equipment);if(snapshot.reminderEnabled!==undefined)setReminder(snapshot.reminderEnabled);setExtras(snapshot.activities);setCloudState("synced")}catch{setCloudState("error")}};
    void load();
  },[user]);
  ...
  const persistWorkout=async()=>{if(!user)return;setCloudState("saving");try{await saveCompletedWorkout(user.id,intensity,activeExercises);setCloudState("synced")}catch{setCloudState("error")}};
  ...
  const addExtra=async()=>{if(!extraName.trim()||!extraAmount.trim())return;const item={id:Date.now(),name:extraName.trim(),amount:extraAmount.trim(),effort:extraEffort,time:new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"})};setExtras(v=>[item,...v]);setExtraOpen(false);if(user){setCloudState("saving");try{await saveExtraActivity(user.id,item);setCloudState("synced")}catch{setCloudState("error")}}flash(user?"额外训练已同步":"额外训练已保存在演示会话")};
  const saveBodyLog=async()=>{if(!user){flash("演示模式不会云端保存");return}setCloudState("saving");try{await saveBodyAndProfile(user.id,{weight:Number(weight),waist:Number(waist),sleep:Number(sleep),water,equipment,reminderEnabled:reminder});setCloudState("synced");flash("今天的身体记录已同步")}catch{setCloudState("error");flash("同步失败，请稍后重试")}};
  const saveReadiness=async(input:ReadinessInput,result:ReadinessRecommendation)=>{setIntensity(result.intensity);if(!user){flash(`已采用${result.intensity}建议（演示模式）`);return}setCloudState("saving");try{await saveDailyReadiness(user.id,input,result);setCloudState("synced");flash("今日训练已经自适应调整")}catch{setCloudState("error");flash("状态保存失败")}};
